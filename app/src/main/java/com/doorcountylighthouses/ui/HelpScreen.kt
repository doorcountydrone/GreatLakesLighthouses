...
fun HelpScreen(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        HelpCard(
            ...
            body = "Great Lakes Lighthouses is a wall map of lights. Each light matches a real lighthouse: white or red, steady or flashing, the same rhythm the lighthouse uses on the water.\n\n" +
                "You do not need to program anything. Use this app, or open the map’s page in a phone or computer browser. Both do the same jobs: pick which lights are on the strip, and set Wi-Fi, brightness, and sleep.",
        )
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                Step(
                    title = "1. Connect to the map’s Wi-Fi",
                    body = "The first time you plug the map in, it makes its own Wi-Fi named GreatLakes-Setup. Password: door1234. Join that network on your phone. Turn off mobile data if your phone keeps leaving Wi-Fi.\n\n" +
                ...
                Step(
                    ...
                    body = "Tap Fetch from Pico to see what is already on the map. A finished Door County map already has Kewaunee through Rock Island. Save to Pico writes your list. List order is the order of the lights on the strip, south to north.",
                )
            }
        }
        ...
        HelpCard(
            ...
            body = "• Pico address: 192.168.4.1 on GreatLakes-Setup. After the map joins home Wi-Fi, use the map’s home-network address.\n\n" +
        ...
        HelpCard(
            title = "Finished map vs building your own",
            body = "A finished Door County map already has the lights programmed. You do not need to add or remove any.\n\n" +
                "Adding, skipping, or reordering is for a custom map. Put lights in the same order as the strip on the board. No programming — just the app or the browser page.",
        )
        ...
        HelpCard(
            ...
            body = "• Can’t connect: join GreatLakes-Setup (password door1234) and use address 192.168.4.1. Turn off mobile data so the phone stays on that Wi-Fi.\n\n" +
                "• Fetch or Save failed: your phone must be on the same Wi-Fi as the map. On setup Wi-Fi that is GreatLakes-Setup. After setup, both must be on your home Wi-Fi, and the Pico address must be the map’s home-network address.\n\n" +
                "• Type http:// in front of the address if the page won’t open.\n\n" +
                "• Phone hotspot: putting the map on a phone hotspot that uses cellular often fails. Use home or hotel Wi-Fi when you can.",
        )

        HelpCard(
            title = "The lights",
            body = "• A light stays dark: check that it is set to Use, not Skip. Check sleep and weekend off times.\n\n" +
                "• Sleep never turns the lights off: the map needs home Wi-Fi so it can set the clock, and the time offset must match your local time.\n\n" +
                "• Catalog search is empty: use Restore defaults or Add custom. On a finished map the catalog is already there.",
        )
