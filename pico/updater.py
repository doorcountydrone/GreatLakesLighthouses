# OTA updater: check version.json on GitHub, download and stage files.
# Install runs only when the user confirms (app, browser, or setup-button tap).
# boot.py swaps *_new.* onto the live names on the next boot.
...
_DOWNLOAD_CHUNK = 512
_ALLOWED_EXT = (".py", ".html")


def _staging_file_looks_like_html_error(path, dest_name):
    try:
...
def check_for_new_version(current_version):
    try:
        ...
        try:
            ...
            print("OTA: version.json is not valid JSON")
            return False, None
        remote_ver = info.get("version", "0.0.0")
        files = info.get("files", [])
        if not files:
            print("OTA: version.json has no files list")
            return False, None
...
def install_pending_update(version_info):
    ...
    try:
        ...
        for entry in files:
            name = entry.get("name")
            url = entry.get("url")
            if not _file_ok(name) or not url:
                continue
            temp_name = _temp_name(name)
            print("OTA downloading", url, "->", temp_name)
            if not _download_url_to_file(url, temp_name, name, 30, "download " + str(name)):
                print("OTA install aborted (bad response for", str(name) + ")")
                return False
            pending_files.append({"name": name, "temp": temp_name})
        if not pending_files:
            print("OTA install: nothing to stage")
            return False
        pending = {
